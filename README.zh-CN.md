...
这个开源学习版的核心思路，不是复述一套抽象人格理论，而是把公开页面里实际运行的判定逻辑拆开说明：

- 测试主干由 30 道常规题组成，对应 15 个属性维度，每个维度由 2 道题共同决定
- 每道题的选项会把当前维度推向不同方向；两道题合并后，最终收敛成该维度的 `L / M / H` 三档之一
- 全部维度计算完成后，会生成一个 15 维人格向量
- 这个人格向量会与页面内置的 25 个人格模板做相似度比较，取最接近的结果作为正常人格输出
- 运行时还存在特殊分支判断：例如饮酒相关分支可能触发隐藏题，某些条件下还会覆盖正常结果，直接返回特殊人格
- 当最终向量与所有模板都不够接近时，会进入低相似度兜底结果

换句话说，这个测试本质上是一个“多维离散向量打分 + 模板最近邻匹配 + 特殊规则覆盖”的分类流程。

## 功能亮点

- 人格向量分析：测试过程中提供实时属性向量监视器。滚动到某一题时，会锁定这道题影响的维度；悬停或选择答案时，可以直接预览本题作答前后的向量变化
- 结果复现与解释：输出最终人格、Top 3 相近人格、维度向量和简要匹配说明，便于学习其分类过程
- 一致性校验：本地评分模型可与上游页面 `computeResult()` 做对照，确保重建逻辑不是凭空猜测
- 移动端适配：题目区、进度区、向量面板和结果页都针对手机屏幕重新排布，保证在窄屏下依然可读、可操作
- GitHub Pages 部署：站点是纯静态前端，可直接公开部署，便于演示和研究
